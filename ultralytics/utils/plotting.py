...
class Annotator:
    ...
    def __init__(
        ...
        if self.pil:  # use PIL
            ...
            self.sf = font_size if font_size is not None else self.lw / 3  # font scale
        # Pose
        self.skeleton = [
            [16, 14],
            [14, 12],
...
def output_to_target(output, max_det=300):
    ...
    # targets layout is [batch_id, class_id, x, y, w, h, conf]; keep tuple order aligned with plot_images().
    return targets[:, 0], targets[:, 1], targets[:, 2:6], targets[:, 6]


def plot_distill_results(file: str, family: str, on_plot: Optional[Callable] = None):
    """
    Plot distillation training results from results.csv.

    Args:
        file (str): Path to results.csv file.
        family (str): Model family, either 'yolomm' or 'rtdetrmm'.
    ...
    # 固定列白名单 -- output 蒸馏子图 + feature 蒸馏子图
    DISTILL_COLS = {
        "yolomm": [
            "train/d_out", "train/d_out_cls", "train/d_out_loc",
            "train/d_feat",
            "train/d_feat_fg", "train/d_feat_bg",
            "train/d_feat_cwd", "train/d_feat_ctx",
        ],
        "rtdetrmm": [
            "train/d_out", "train/d_out_cls", "train/d_out_box",
            "train/d_feat",
            "train/d_feat_fg", "train/d_feat_bg",
            "train/d_feat_cwd", "train/d_feat_ctx",
        ],
    ...
    target_cols = DISTILL_COLS.get(family.lower())
    if not target_cols:
        LOGGER.warning(f"Unknown family '{family}', skip distill_results.png")
        return
    ...
    # 检查哪些列存在
    available_cols = [col for col in target_cols if col in columns]
    if not available_cols:
        LOGGER.info("No distillation columns found, skip distill_results.png")
        return

    # 动态布局: 按实际可用列数量确定子图数
    n_cols = len(available_cols)
    n_per_row = min(n_cols, 4)
    n_rows = (n_cols + n_per_row - 1) // n_per_row
    fig, axes = plt.subplots(n_rows, n_per_row, figsize=(4 * n_per_row, 4 * n_rows), tight_layout=True)
    if n_rows * n_per_row == 1:
        axes = [axes]
    else:
    ...
    for i, col in enumerate(available_cols):
        j = columns.index(col)
        y = data.values[:, j].astype("float")
        axes[i].plot(x, y, marker=".", linewidth=2, markersize=8)
        axes[i].plot(x, gaussian_filter1d(y, sigma=3), ":", linewidth=2)
    ...
    fname = save_dir / "distill_results.png"
    fig.savefig(fname, dpi=200)
    plt.close()
    LOGGER.info(f"Distillation results saved to {fname}")
    if on_plot:
        on_plot(fname)
