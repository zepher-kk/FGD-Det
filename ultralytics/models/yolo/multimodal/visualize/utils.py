...
class HookManager:
    ...
    def __init__(self, model: nn.Module):
        ...
        self._vishook_modules: List[nn.Module] = []  # track modules we attached hooks to
    ...
    def register_forward_hook(
        ...
        # 在模块上维护 vishook 句柄与标签，确保隔离
        if not hasattr(module, "_vishook_handles"):
            module._vishook_handles = []  # type: ignore[attr-defined]
        if not hasattr(module, "_vishook_tags"):
            module._vishook_tags = set()  # type: ignore[attr-defined]
        module._vishook_tags.add(tag)  # type: ignore[attr-defined]
        # 使用提供的名称作为特征键（若未提供则退回到模块 id），以便下游能够得到可读的层名
        module._vishook_key = (name if name is not None else id(module))  # type: ignore[attr-defined]

        handle = module.register_forward_hook(hook_fn)
        module._vishook_handles.append(handle)  # type: ignore[attr-defined]
    ...
    def register_backward_hook(
        ...
        if not hasattr(module, "_vishook_tags"):
            module._vishook_tags = set()  # type: ignore[attr-defined]
        module._vishook_tags.add(tag)  # type: ignore[attr-defined]
        module._vishook_key = id(module)  # type: ignore[attr-defined]
        ...
        for mod in self._vishook_modules:
            try:
                ...
                if hasattr(mod, "_vishook_handles"):
                    mod._vishook_handles = []  # type: ignore[attr-defined]
                if hasattr(mod, "_vishook_tags"):
                    mod._vishook_tags = set()  # type: ignore[attr-defined]
