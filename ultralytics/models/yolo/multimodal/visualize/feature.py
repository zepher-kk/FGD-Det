...
class FeatureMapVisualizer:
    ...
    def _visualize_single_modality(
        ...
        **kwargs
        ...
        # Determine layer_idx summary for result metadata/API
        if layers is None:
            layer_idx_val = 'auto'
        else:
    ...
    def _get_default_layers(self) -> List[str]:
        ...
        # Use reasonable defaults if none found
    ...
    def _extract_features(
        ...
        """
        Extract feature maps from specified layers using hooks; on GPU OOM, switch to a CPU extraction path.
        ...
        try:
            # Try feature extraction on current device (Fail‑Fast: no auto CPU fallback)
            input_tensor = input_tensor.to(original_device)
            with torch.no_grad():
                _ = self.model(input_tensor)
            features = self.hook_manager.get_features()
        except RuntimeError as e:
            # Fail‑Fast: 禁止 GPU→CPU 自动切换，给出可执行修复建议
    ...
    def _render_grid(
        ...
        **kwargs
        ...
        # Get colormap (support 'none'/'gray' to avoid pseudocolor)
        ...
        for idx, item in enumerate(selected_features):
            ...
            if cmap is None:
                # Keep grayscale, convert to RGB for consistent 3-channel output
                feat_colored = cv2.cvtColor(feat_resized, cv2.COLOR_GRAY2RGB)
            else:
                feat_colored = cv2.applyColorMap(feat_resized, cmap)  # OpenCV returns BGR
