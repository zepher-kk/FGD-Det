...
class YOLOMM(Model):
    ...
    def val_tre(
        ...
        **kwargs,
        ...
        """
        Triple-Reference Evaluation (TRE) for multi-modal fusion robustness.

        Runs model inference at multiple IR offsets, then classifies each
        prediction against three reference frames (RGB-GT, X-GT, Union-GT)
        and computes AP_robust, MRR, SDR, RFS per offset, plus integrated
        RFS_total over the offset range.

        Args:
            offsets: list of (dx, dy) tuples, e.g. [(0,0), (5,5), (10,10)].
                     Defaults to [(0,0), (3,3), (6,6), (9,9), (12,12), (15,15)].
            **kwargs: standard val args (data, imgsz, batch, conf, iou, device, etc.)

        Returns:
            dict with keys:
                - offsets: list of (dx, dy) evaluated
                - per_offset: list of per-offset result dicts
                - RFS_total_mAP50: integrated RFS (mAP50-based)
                - RFS_total_mAP50_95: integrated RFS (mAP50-95-based)

        Example:
            >>> model = YOLOMM('yolo11n.pt')
            >>> results = model.val_tre(
            ...     data='m3fd.yaml',
            ...     offsets=[(0,0), (5,5), (10,10), (15,15)]
            ... )
            >>> print(f"RFS_total: {results['RFS_total_mAP50_95']:.4f}")
        """
        from ultralytics.models.yolo.multimodal.tre_val import TREValidator

        self._check_is_pytorch_model()

        kwargs.setdefault("rect", True)
        kwargs.setdefault("plots", False)
        kwargs.setdefault("save_json", False)
        kwargs.setdefault("mode", "cocoval")
        ...
        results = validator.validate_tre(offsets=offsets, model=self.model)
        self.metrics = results
        return results
