...
def random_shift_ir(ir_img, max_offset=15):
    """
    对 IR 图像施加随机平移，模拟物理空间未对齐。

    Args:
        ir_img (np.ndarray): 输入的红外图像 (H, W, C)
        max_offset (int): 最大偏移像素，例如 15 表示在 [-15, 15] 之间随机平移

    Returns:
        np.ndarray: 随机平移并补灰边后的红外图像
    """
    # 1. 在 [-max_offset, max_offset] 范围内生成完全随机的 x 和 y 偏移量
    tx = random.randint(-max_offset, max_offset)
    ty = random.randint(-max_offset, max_offset)

    # 如果随机到 0 偏移，直接返回原图，节省计算
    if tx == 0 and ty == 0:
        return ir_img
    ...
    # 3. 执行平移，移出的部分用 YOLO 默认灰边 (114, 114, 114) 填充
    h, w = ir_img.shape[:2]
