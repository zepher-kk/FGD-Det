...
class IRRandomShift(BaseMultiModalTransform):
    """
    IR 专属大范围随机平移增强，模拟 RGB-IR 传感器物理空间未对齐。

    设计要点：
    - 仅平移 IR/X 模态（默认 target='x'），RGB 保持不变
    - 使用 cv2.warpAffine + 灰边填充 (114)，与 TRE 评估保持完全一致
    - GT 框不移动 → 训练信号：RGB 是空间锚点，利用 IR 纹理但不要被其位置带偏
    - TRE 兼容：训练偏移范围覆盖 TRE 评估范围 (0~15px)

    Args:
        p: 触发概率 (0~1)
        max_offset: 最大偏移像素，tx, ty ∈ [-max_offset, max_offset]
        border: 边界填充值，默认 114 (YOLO 灰边)
        target: 平移目标模态，'x' (默认，IR模态) 或 'rgb'
    """

    def __init__(self, p: float = 0.5, max_offset: int = 15, border: int = 114, target: str = "x"):
        assert 0.0 <= p <= 1.0
        assert target in {"x", "rgb"}
        self.p = p
    ...
    def __call__(self, labels):
        ...
        tx = random.randint(-self.max_offset, self.max_offset)
        ty = random.randint(-self.max_offset, self.max_offset)
        if tx == 0 and ty == 0:
            return labels
        ...
        border_value = (self.border,) * x.shape[2] if x.ndim == 3 and x.shape[2] > 0 else self.border

        if self.target == "x":
            if x.ndim == 2:
                x = x[:, :, np.newaxis]
                shifted = cv2.warpAffine(x, M, (W, H), borderValue=(self.border,))
                x = shifted[:, :, np.newaxis] if shifted.ndim == 2 else shifted
            else:
                x = cv2.warpAffine(x, M, (W, H), borderValue=border_value)
        else:  # target == 'rgb'
            rgb = cv2.warpAffine(rgb, M, (W, H), borderValue=(self.border, self.border, self.border))
...
def mm_transforms(dataset, imgsz: int, hyp) -> Compose:
    ...
    # IR 大范围随机平移（模拟传感器未对齐，TRE 兼容训练）
    # 仅平移 IR/X 模态，GT 保持 RGB 坐标系不动
    if getattr(hyp, 'mm_ir_shift_p', 0.0) > 0.0:
        tfl.append(
            IRRandomShift(
                p=getattr(hyp, 'mm_ir_shift_p', 0.5),
                max_offset=getattr(hyp, 'mm_ir_shift_max', 15),
                border=getattr(hyp, 'mm_ir_shift_border', 114),
                target=getattr(hyp, 'mm_ir_shift_target', 'x'),
            )
        )
...
def mm_seg_transforms(dataset, imgsz: int, hyp) -> Compose:
    ...
    # IR 大范围随机平移（与 mm_transforms 保持一致）
    if getattr(hyp, 'mm_ir_shift_p', 0.0) > 0.0:
        tfl.append(
            IRRandomShift(
                p=getattr(hyp, 'mm_ir_shift_p', 0.5),
