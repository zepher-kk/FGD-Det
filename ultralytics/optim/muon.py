...
"""
MuSGD and Muon optimizers for neural network training.

This module implements the Muon optimizer (inspired by Moonshot AI's Kimi K2 LLM training)
and the hybrid MuSGD optimizer that combines Muon with SGD updates.

Key features:
    - Newton-Schulz orthogonalization for gradient updates
    - Hybrid Muon+SGD strategy for enhanced stability
    - Support for 2D+ parameter tensors (conv weights, linear layers)
"""
...
def zeropower_via_newtonschulz5(G: torch.Tensor, eps: float = 1e-7) -> torch.Tensor:
    ...
    maximize convergence slope at zero, producing a result similar to UV^T from SVD.
    ...
    if G.size(0) > G.size(1):
        X = X.T
    # Newton-Schulz iteration with fixed coefficients (5 steps)
    for a, b, c in [
...
def muon_update(grad: torch.Tensor, momentum: torch.Tensor, beta: float = 0.95, nesterov: bool = True) -> torch.Tensor:
    ...
    and then orthogonalizes the update using Newton-Schulz iterations.
    ...
    Notes:
        ...
        - 4D tensors (conv filters) are reshaped to 2D for orthogonalization.
        - Final update is scaled by sqrt(max(dim[-2], dim[-1])).
    """
...
class MuSGD(optim.Optimizer):
    ...
    Args:
        ...
        momentum (float): Momentum factor for SGD. Default: 0.0.
        weight_decay (float): Weight decay (L2 penalty). Default: 0.0.
        nesterov (bool): Whether to use Nesterov momentum. Default: False.
        use_muon (bool): Whether to enable Muon updates. Default: False.
        muon (float): Scaling factor for Muon component. Default: 0.5.
        sgd (float): Scaling factor for SGD component. Default: 0.5.
    ...
    def step(self, closure=None):
        ...
        'use_muon' flag in each parameter group.
...
class Muon(optim.Optimizer):
    ...
    with orthogonalization via Newton-Schulz iterations.
    ...
    def step(self, closure=None):
        ...
        Args:
            closure (Callable[[], torch.Tensor] | None, optional): A closure that reevaluates
                the model and returns the loss. Default: None.
