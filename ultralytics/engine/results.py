...
class Results(SimpleClass, DataExportMixin):
    ...
    def plot(
        ...
        # Draw source filename on image
        if show_filename and self.path:
            im_result = annotator.im if pil else annotator.result()
            fname = Path(self.path).name[:60]
            fs = annotator.sf if hasattr(annotator, "sf") else 0.5
            ft = annotator.tf if hasattr(annotator, "tf") else 1
            w, h = cv2.getTextSize(fname, 0, fs, ft)[0]
            pad = 5
            cv2.rectangle(im_result, (pad, pad), (pad + w + 2, pad + h + 6), (0, 0, 0), -1)
            cv2.putText(im_result, fname, (pad + 1, pad + h + 1), 0, fs, (255, 255, 255), ft, cv2.LINE_AA)
            return im_result
