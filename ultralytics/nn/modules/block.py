...
class SPPF(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, k: int = 5, n: int = 3, shortcut: bool = False):
        ...
        Args:
            ...
            n (int): Number of pooling iterations. Default is 3 (compatible with YOLO11).
            shortcut (bool): Whether to add shortcut connection. Default is False.
                When True, requires c1 == c2 for residual connection.

        Raises:
            ValueError: If shortcut=True but c1 != c2.

        Notes:
            - YOLO11 uses SPPF(c1, c2, 5) with n=3 by default
            - YOLO26 uses SPPF(c1, c2, 5, 3, True) with shortcut enabled
            - This module is equivalent to SPP(k=(5, 9, 13)) when n=3
        """
        super().__init__()
        if shortcut and c1 != c2:
            raise ValueError(f"SPPF shortcut requires c1 == c2, but got c1={c1}, c2={c2}")
...
class Proto26(Proto):
    """YOLO26 mask Proto module for segmentation models with multi-scale feature fusion."""

    def __init__(self, ch: tuple = (), c_: int = 256, c2: int = 32, nc: int = 80):
        """Initialize Proto26 with multi-scale feature refinement and semantic segmentation head.
    ...
    def forward(self, x: torch.Tensor, return_semseg: bool = True) -> torch.Tensor:
        """Perform forward pass with multi-scale feature fusion."""
        feat = x[0]
...
class RealNVP(nn.Module):
    """RealNVP flow-based generative model for pose estimation.
