...
from ultralytics.nn.backbone.TransNext import AggregatedAttention, get_relative_position_cpb
...
class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=7):
        ...
        # 这就是报错的那个卷积，它严格要求输入是 2 通道
        self.conv1 = nn.Conv2d(2, 1, kernel_size, padding=padding, bias=False)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        # 修复 Bug 的关键：必须在这里先计算平均和最大，拼成 2 通道！
...
class CBAM(nn.Module):
    # 第一参数必须是 channels，完美适配我们的 eval(attn_type)(hidden_channels)
