...
__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "v26Detect", "YOLOEDetect", "YOLOESegment", "v8Detect", "Segment26", "OBB26", "Pose26"
...
class v8Detect(Detect):
    """
    纯 YOLOv8 风格检测头（分类分支为标准 3x3 Conv 堆叠），与当前默认 Detect 的 DWConv+Conv 分类分支区分。

    用法：在模型 YAML 的 head 最后一层使用 `v8Detect`，其余保持与 `Detect` 一致。

    与 Detect 的差异：
    - 将 `legacy=True` 固定开启，使分类分支 `cv3` 采用两层 3x3 Conv + 1x1 输出（与传统 YOLOv8 保持一致）。
    - 其余逻辑（DFL、锚点生成、解码、推理路径等）与父类 Detect 保持一致。
    """

    def __init__(self, nc: int = 80, ch: Tuple = ()):  # noqa: D401
        # 在父类构造前设置 legacy，确保父类根据 legacy 构建分类分支
        self.legacy = True
...
class RTDETRDecoder(nn.Module):
    ...
    export = False  # export mode
    end2end = True   # RT-DETR uses end-to-end detection
...
class v26Detect(Detect):
    """
    YOLO26 Detection head with end-to-end detection and simplified DFL.

    This class implements the YOLO26 detection head featuring:
    - Default end-to-end detection mode (one2many + one2one dual branches)
    - Simplified DFL with reg_max=1 for reduced computation
    - Compatible with both single-modal and multi-modal (YOLOMM) architectures

    Attributes:
        end2end (bool): End-to-end detection mode, default True.
        reg_max (int): DFL channels, default 1 for YOLO26.
        cv2 (nn.ModuleList): Convolution layers for box regression.
        cv3 (nn.ModuleList): Convolution layers for classification.
        one2one_cv2 (nn.ModuleList): One-to-one convolution layers for box regression.
        one2one_cv3 (nn.ModuleList): One-to-one convolution layers for classification.
        dfl (nn.Module): DFL module (Identity when reg_max=1).

    Methods:
        __init__: Initialize v26Detect with YOLO26-specific defaults.
        forward: Perform forward pass with end-to-end detection.
        fuse: Remove one2many head for inference optimization.

    Examples:
        Create a v26Detect head for YOLO26
        >>> v26_detect = v26Detect(nc=80, ch=(256, 512, 1024))
        >>> x = [torch.randn(1, 256, 80, 80), torch.randn(1, 512, 40, 40), torch.randn(1, 1024, 20, 20)]
        >>> outputs = v26_detect(x)

        Create with custom reg_max (not recommended)
        >>> v26_detect = v26Detect(nc=80, ch=(256, 512, 1024), reg_max=4)
    """

    end2end = True  # YOLO26 默认启用端到端检测

    def __init__(self, nc: int = 80, ch: Tuple = (), reg_max: int = 1):
        """
        Initialize the YOLO26 detection head with end-to-end detection and simplified DFL.

        Args:
            nc (int): Number of classes. Default is 80.
            ch (tuple): Tuple of channel sizes from backbone feature maps.
            reg_max (int): DFL channels, default 1 for YOLO26 simplified DFL.
                - reg_max=1: Simplified regression, direct distance prediction (YOLO26 default)
                - reg_max>1: Traditional DFL with distribution modeling (YOLO11 style)

        Raises:
            ValueError: If reg_max <= 0.

        Notes:
            - YOLO26 uses reg_max=1 by default for reduced computation and inference latency.
            - The end2end=True is hardcoded at class level for YOLO26 behavior.
            - When reg_max=1, DFL module becomes nn.Identity() for efficiency.
        """
        if reg_max <= 0:
        ...
        # 调用父类构造 (使用 object.__init__ 跳过 Detect.__init__)
        nn.Module.__init__(self)

        # Initialize conv layers with YOLO26-specific channels
        c2 = max((16, ch[0] // 4, self.reg_max * 4))
        c3 = max(ch[0], min(self.nc, 100))

        # Box regression head (cv2)
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch
        )

        # Classification head (cv3) - 使用轻量级DWConv结构
        self.cv3 = nn.ModuleList(
            nn.Sequential(
                nn.Sequential(DWConv(x, x, 3), Conv(x, c3, 1)),
                nn.Sequential(DWConv(c3, c3, 3), Conv(c3, c3, 1)),
        ...
        # DFL module: Identity when reg_max=1 (YOLO26), standard DFL otherwise
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()

        # End-to-end detection: create one2one branches
        if self.end2end:
            self.one2one_cv2 = copy.deepcopy(self.cv2)
            self.one2one_cv3 = copy.deepcopy(self.cv3)

    def forward(self, x: List[torch.Tensor]) -> Union[dict, Tuple]:
        """
        Perform forward pass of the v26Detect module with end-to-end detection.

        Args:
            x (List[torch.Tensor]): Input feature maps from different levels (P3, P4, P5).

        Returns:
            Training mode:
                dict: {"one2many": List[Tensor], "one2one": List[Tensor]}
                    - one2many: For training with multiple anchor matches
                    - one2one: For end-to-end detection without NMS

            Inference mode:
                tuple: (predictions, raw_outputs) if not export else predictions
                    - predictions: Processed Nx6 tensor [x, y, w, h, conf, cls]
                    - raw_outputs: Dict with one2many and one2one raw features

        Notes:
            - one2one branch uses detached features to prevent gradient backprop to backbone
            - Inference automatically uses one2one branch for NMS-free detection
        """
        # 为 one2one 分支准备 detached 特征
        x_detach = [xi.detach() for xi in x]
...
class Segment26(Segment):
    """YOLO26 Segment head for segmentation models with Proto26 multi-scale fusion."""

    def __init__(self, nc: int = 80, nm: int = 32, npr: int = 256, ch: Tuple = ()):
        """Initialize Segment26 with Proto26 for multi-scale feature fusion.
    ...
    def forward(self, x: List[torch.Tensor]) -> Union[Tuple, List[torch.Tensor]]:
        ...
        if self.training:
            return det_out, mc, proto
        return (torch.cat([det_out, mc], 1), proto) if self.export else (torch.cat([det_out[0], mc], 1), (det_out[1], mc, proto))

    def fuse(self) -> None:
        """Remove semantic segmentation head for inference optimization."""
        super().fuse()
        if hasattr(self.proto, "fuse"):
            self.proto.fuse()

class OBB26(OBB):
    """YOLO26 OBB detection head with raw angle output (no sigmoid transformation)."""

    def __init__(self, nc: int = 80, ne: int = 1, ch: Tuple = ()):
        """Initialize OBB26 with raw angle prediction.

        Args:
            nc (int): Number of classes.
            ne (int): Number of extra parameters (angle).
            ch (tuple): Tuple of channel sizes from backbone feature maps.
        """
    ...
    def forward(self, x: List[torch.Tensor]) -> Union[torch.Tensor, Tuple]:
        ...
        if not self.training:
            self.angle = (angle.sigmoid() - 0.25) * math.pi
        x = Detect.forward(self, x)
        if self.training:
            return x, angle
        angle_inf = (angle.sigmoid() - 0.25) * math.pi
        return torch.cat([x, angle_inf], 1) if self.export else (torch.cat([x[0], angle_inf], 1), (x[1], angle))

class Pose26(Pose):
    """YOLO26 Pose head with RealNVP flow model and keypoint uncertainty estimation."""

    def __init__(self, nc: int = 80, kpt_shape: Tuple = (17, 3), ch: Tuple = ()):
        """Initialize Pose26 with RealNVP flow model for keypoint estimation.

        Args:
            nc (int): Number of classes.
            kpt_shape (tuple): Number of keypoints and dimensions.
            ch (tuple): Tuple of channel sizes from backbone feature maps.
        """
    ...
    def forward(self, x: List[torch.Tensor]) -> Union[torch.Tensor, Tuple]:
        """Perform forward pass with keypoint and uncertainty prediction."""
        bs = x[0].shape[0]
