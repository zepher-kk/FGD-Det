"""
LWGA - 局部窗口全局注意力 (Local-Window Global Attention)
DRFD - 双分支下采样 (Dual-Branch Reduction and Fusion Downsampling)
PA - 点注意力 (Point Attention)

论文: Local-Window Global Attention for Object Detection
期刊/会议: arXiv 2501.10040
...
class DRFD(nn.Module):
    ...
    def forward(self, x):
        x = self.conv(x)
        max_ = self.norm_m(self.max_m(x))
        conv = self.norm_c(self.act_c(self.conv_c(x)))
        x = torch.cat([conv, max_], dim=1)
        x = self.fusion(x)
        return x
...
class LWGA(nn.Module):
    ...
    def __init__(self,
        ...
        _drop_path = DropPath(drop_path) if (DropPath is not None and drop_path > 0.) else nn.Identity()
        ...
        self.drop_path = DropPath(drop_path) if DropPath is not None else nn.Identity()
