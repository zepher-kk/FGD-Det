...
# - 保持上游 RTDETR-main 的**模块/函数命名**（SwinTransformer_Tiny），便于 YAML 直接迁移。
...
def SwinTransformer_Tiny(in_chans: int = 3, weights: str = "", pretrained: bool = False, **kwargs) -> nn.Module:
    if kwargs:
        raise ValueError(f"SwinTransformer_Tiny 不支持额外 kwargs：{sorted(kwargs.keys())}")
    m = _TimmFeaturesBackbone(
        in_chans=in_chans,
