...
class Attention4DDownsample(torch.nn.Module):
    def __init__(self, dim=384, key_dim=16, num_heads=8, attn_ratio=4, resolution=7, out_dim=None, act_layer=None):
        ...
        self.v = nn.Sequential(nn.Conv2d(dim, self.num_heads * self.d, 1), nn.BatchNorm2d(self.num_heads * self.d))
        self.v_local = nn.Sequential(
            nn.Conv2d(self.num_heads * self.d, self.num_heads * self.d, kernel_size=3, stride=2, padding=1, groups=self.num_heads * self.d),
            nn.BatchNorm2d(self.num_heads * self.d),
        )
...
class FFN(nn.Module):
    def __init__(self, dim, pool_size=3, mlp_ratio=4.0, act_layer=nn.GELU, drop=0.0, drop_path=0.0, use_layer_scale=True, layer_scale_init_value=1e-5):
        super().__init__()
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, mid_conv=True)
...
class _InputProjBackbone(nn.Module):
    ...
    def __init__(self, in_chans: int, backbone: nn.Module, *, expected_imgsz: Optional[int], name: str):
        super().__init__()
        self.in_chans = int(in_chans)
        if self.in_chans <= 0:
            raise ValueError(f"{name}: in_chans 必须为正整数，当前={self.in_chans}")
        self.expected_imgsz = int(expected_imgsz) if expected_imgsz is not None else None
        self.name = str(name)
    ...
    def forward(self, x: torch.Tensor):
        if x.ndim != 4:
            raise ValueError(f"{self.name}: 输入必须为 4D BCHW")

        orig_h, orig_w = int(x.shape[-2]), int(x.shape[-1])
        tgt = self.expected_imgsz  # EFv2 初始化时锁定的 resolution

        # EFv2 Attention4D 对输入尺寸硬编码（self.N = resolution²，attention_bias_idxs 预计算）
        # 必须跑在 tgt×tgt 方阵上。若 dataloader 给进来的不是 tgt×tgt
        # （例如 val rect=True 或 imgsz 不一致），先 resize 输入到 tgt×tgt。
        need_resize = (tgt is not None) and ((orig_h != tgt) or (orig_w != tgt))
        if need_resize:
            x_proc = F.interpolate(x, size=(tgt, tgt), mode="bilinear", align_corners=False)
        else:
            x_proc = x

        x_proc = self.input_proj(x_proc)
        outs = self.backbone_impl(x_proc)  # list[Tensor]：各 stage 尺寸 tgt/4, tgt/8, tgt/16, tgt/32

        # 若输入被 resize 过，把各 stage 输出按 "原始 hw ÷ stage_stride" 反向采样回来，
        # 保持和 RGB 路径 / 融合模块尺寸对齐。
        if need_resize:
            aligned = []
            for feat in outs:
                feat_hw = int(feat.shape[-1])
                stride = max(1, tgt // feat_hw)  # P2=4, P3=8, P4=16, P5=32
                new_h = max(1, orig_h // stride)
                new_w = max(1, orig_w // stride)
                if (new_h, new_w) != (int(feat.shape[-2]), int(feat.shape[-1])):
                    feat = F.interpolate(feat, size=(new_h, new_w), mode="bilinear", align_corners=False)
                aligned.append(feat)
            outs = aligned
...
def efficientformerv2_s0(in_chans: int = 3, weights: str = "", resolution: int = 640, **kwargs):
    ...
    if weights:
        pretrained_weight = torch.load(weights, map_location="cpu")["model"]
        model.load_state_dict(update_weight(model.state_dict(), pretrained_weight))
    return _InputProjBackbone(in_chans=in_chans, backbone=model, expected_imgsz=int(resolution), name="efficientformerv2_s0")


def efficientformerv2_s1(in_chans: int = 3, weights: str = "", resolution: int = 640, **kwargs):
    model = EfficientFormerV2(
    ...
    if weights:
        pretrained_weight = torch.load(weights, map_location="cpu")["model"]
        model.load_state_dict(update_weight(model.state_dict(), pretrained_weight))
    return _InputProjBackbone(in_chans=in_chans, backbone=model, expected_imgsz=int(resolution), name="efficientformerv2_s1")


def efficientformerv2_s2(in_chans: int = 3, weights: str = "", resolution: int = 640, **kwargs):
    model = EfficientFormerV2(
    ...
    if weights:
        pretrained_weight = torch.load(weights, map_location="cpu")["model"]
        model.load_state_dict(update_weight(model.state_dict(), pretrained_weight))
    return _InputProjBackbone(in_chans=in_chans, backbone=model, expected_imgsz=int(resolution), name="efficientformerv2_s2")


def efficientformerv2_l(in_chans: int = 3, weights: str = "", resolution: int = 640, **kwargs):
    model = EfficientFormerV2(
    ...
    if weights:
        pretrained_weight = torch.load(weights, map_location="cpu")["model"]
        model.load_state_dict(update_weight(model.state_dict(), pretrained_weight))
    return _InputProjBackbone(in_chans=in_chans, backbone=model, expected_imgsz=int(resolution), name="efficientformerv2_l")
